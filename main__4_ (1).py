...
def main_menu(uid, d):
    ...
    # Blue buttons row
    rows.append([
        btn_blue("📊 Bot Statistics", callback="stats:show", emoji_key="chart"),
...
async def cb_premium_package(cq: types.CallbackQuery):
    ...
    save(d)
    await cq.answer()
    text = styled_box("✅ ᴄʀᴇᴅɪᴛꜱ ᴀᴅᴅᴇᴅ",
        f"{ce('check')} {sc('ꜱᴜᴄᴄᴇꜱꜱ!')}\n"
        f"{ce('gift')} {sc('ᴄʀᴇᴅɪᴛꜱ ᴀᴅᴅᴇᴅ')}: <b>+{credits}</b>\n"
        f"{ce('diamond')} {sc('ᴛᴏᴛᴀʟ ʙᴀʟᴀɴᴄᴇ')}: <b>{u['credits']}</b>\n\n"
        f"{ce('info')} {sc('ᴘᴀʏᴍᴇɴᴛ ᴜᴘɪ')}: <code>{PAYMENT_UPI}</code>")
    await cq.message.edit_text(text, reply_markup=back_button("home"), parse_mode="HTML")
...
async def process_custom_credits(msg: types.Message, state: FSMContext):
    try:
        ...
        save(d)
        await state.clear()
        await msg.answer(styled_box("✅ ᴄʀᴇᴅɪᴛꜱ ᴀᴅᴅᴇᴅ",
            f"{ce('check')} {sc('ꜱᴜᴄᴄᴇꜱꜱ!')}\n"
            f"{ce('gift')} {sc('ᴀᴍᴏᴜɴᴛ')}: <b>{credits} Credits</b>\n"
            f"{ce('money')} {sc('ᴘʀɪᴄᴇ')}: <b>₹{price_rs:.2f}</b>\n"
            f"{ce('diamond')} {sc('ᴛᴏᴛᴀʟ ʙᴀʟᴀɴᴄᴇ')}: <b>{u['credits']}</b>\n\n"
            f"{ce('info')} {sc('ᴘᴀʏᴍᴇɴᴛ ᴜᴘɪ')}: <code>{PAYMENT_UPI}</code>"),
            reply_markup=back_button("home"), parse_mode="HTML")
    except:
