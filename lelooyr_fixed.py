...
async def bomber_worker(bot, uid, number, message, count, sched_id=None):
    ...
    try:
        msg = await bot.send_message(uid,
            ...
            f"{ce('chart')} {sc('ꜱᴜᴄᴄᴇꜱꜱ ʀᴀᴛᴇ')}: <b>{rate}%</b>"), parse_mode="HTML", reply_markup=kb([[btn_primary("Return to Dashboard", callback="home", emoji_key="home")]]))
        ...
        try:
            ...
            check_ref_expiry(u)
            save(d)
            user_name = await bot.get_chat(uid)
            user_name = user_name.first_name or user_name.username or str(uid)
            text = (
                f"{DIV}\n"
            ...
            await msg.edit_text(text, reply_markup=main_menu(uid, d), parse_mode="HTML")
        except:
            pass
    except:
...
async def cmd_start(msg: types.Message, state: FSMContext):
    ...
    # STEP 1: Store referral data if link has referrer
    args = msg.text.split()
    if len(args) > 1:
        try:
            ref_uid = int(args[1])
            if ref_uid != uid:
                ...
                if not u.get("referred_by"):
                    # Store pending referral, will be verified after force join
                    if "pending_referrals" not in d:
                        d["pending_referrals"] = {}
                    d["pending_referrals"][str(uid)] = {
                        "referrer_id": ref_uid,
                        "referred_user": user_name,
    ...
    # STEP 3: Force join check - CRITICAL for referral bonus
    if not await check_fj(msg.bot, uid, FORCE_JOIN_CHANNELS):
        await send_fj_ui(msg, FORCE_JOIN_CHANNELS)
        return

    # STEP 4: User joined all channels, now process pending referral if exists
    if "pending_referrals" in d and str(uid) in d["pending_referrals"]:
        pending = d["pending_referrals"][str(uid)]
        referrer_id = pending["referrer_id"]
        
        # Give bonus to BOTH users
        u = get_user(d, uid)
        ru = get_user(d, referrer_id)
        
        # Referred user gets welcome bonus
        u["referred_by"] = referrer_id
        u["credits"] = u.get("credits", 0) + 300  # Welcome bonus
        u["bonus_attacks"] += 1
        
        # Referrer gets reward
        ru["referrals"] += 1
        ru["credits"] = ru.get("credits", 0) + 500  # Referral reward
        ru["bonus_attacks"] += 2
        ru["referral_bonus_expiry"] = (datetime.now() + timedelta(hours=REFERRAL_EXPIRY_HOURS)).isoformat()
        
        # Remove from pending
        del d["pending_referrals"][str(uid)]
        save(d)
        
        # Notify referrer
        try:
            await msg.bot.send_message(referrer_id,
                styled_box("ʀᴇꜰᴇʀʀᴀʟ ʀᴇᴡᴀʀᴅ ✅",
                f"{ce('gift')} {sc(f'{user_name} ᴀᴄᴛɪᴠᴀᴛᴇᴅ!')}\n"
                f"{ce('fire')} {sc('ʙᴏɴᴜꜱ')}: <b>+2 ᴀᴛᴛᴀᴄᴋꜱ</b>\n"
                f"{ce('money')} {sc('ᴄʀᴇᴅɪᴛꜱ')}: <b>+500 ᴄʀᴇᴅɪᴛꜱ</b>\n"
                f"{ce('clock')} {sc('ᴇxᴘɪʀᴇꜱ')}: <b>{REFERRAL_EXPIRY_HOURS}h</b>"), parse_mode="HTML")
        except: pass
...
async def cb_fj_verify(cq: types.CallbackQuery, state: FSMContext):
    ...
    if await check_fj(cq.bot, uid, FORCE_JOIN_CHANNELS):
        ...
        # Process pending referral if user has one
        if "pending_referrals" in d and str(uid) in d["pending_referrals"]:
            pending = d["pending_referrals"][str(uid)]
            referrer_id = pending["referrer_id"]
            ...
            u["referred_by"] = referrer_id
            u["credits"] = u.get("credits", 0) + 300
            u["bonus_attacks"] += 1
            
            ru["referrals"] += 1
            ru["credits"] = ru.get("credits", 0) + 500
            ru["bonus_attacks"] += 2
            ru["referral_bonus_expiry"] = (datetime.now() + timedelta(hours=REFERRAL_EXPIRY_HOURS)).isoformat()
            ...
            # Notify referrer
            try:
                await cq.bot.send_message(referrer_id,
                    styled_box("ʀᴇꜰᴇʀʀᴀʟ ʀᴇᴡᴀʀᴅ ✅",
                    f"{ce('gift')} {sc(f'{user_name} ᴀᴄᴛɪᴠᴀᴛᴇᴅ!')}\n"
                    f"{ce('fire')} {sc('ʙᴏɴᴜꜱ')}: <b>+2 ᴀᴛᴛᴀᴄᴋꜱ</b>\n"
                    f"{ce('money')} {sc('ᴄʀᴇᴅɪᴛꜱ')}: <b>+500 ᴄʀᴇᴅɪᴛꜱ</b>\n"
            ...
            await cq.answer(sc("✅ ʀᴇꜰᴇʀʀᴀʟ ᴀᴄᴛɪᴠᴀᴛᴇᴅ! ᴘʟᴜꜱ ᴄʀᴇᴅɪᴛꜱ +300"), show_alert=True)
        else:
            await cq.answer(sc("✅ ᴠᴇʀɪꜰɪᴇᴅ!"), show_alert=True)
        
        await cb_home(cq, state)
    else:
        await cq.answer(sc("❌ ɴᴏᴛ ᴀʟʟ ᴄʜᴀɴɴᴇʟꜱ ᴊᴏɪɴᴇᴅ!"), show_alert=True)
...
async def cb_refer(cq: types.CallbackQuery):
    ...
    # Create both refer link and invite link (same thing)
    refer_link = f"https://t.me/{bot_un}?start={uid}"
    ...
    text = styled_box("ʀᴇꜰᴇʀʀᴀʟ ꜱʏꜱᴛᴇᴍ",
        f"{ce('link')} {sc('ʏᴏᴜʀ ɪɴᴠɪᴛᴇ ʟɪɴᴋ')}:\n<code>{refer_link}</code>\n\n"
        f"{ce('gift')} {sc('ʀᴇꜰᴇʀʀᴇʀ ʙᴏɴᴜꜱ')}: <b>+500 ᴄʀᴇᴅɪᴛꜱ +2 ᴀᴛᴛᴀᴄᴋꜱ</b>\n"
        f"{ce('diamond')} {sc('ʀᴇꜰᴇʀʀᴇᴅ ᴜꜱᴇʀ ʙᴏɴᴜꜱ')}: <b>+300 ᴄʀᴇᴅɪᴛꜱ +1 ᴀᴛᴛᴀᴄᴋ</b>\n"
        f"{ce('clock')} {sc('ᴅᴜʀᴀᴛɪᴏɴ')}: <b>{REFERRAL_EXPIRY_HOURS}ʜ (ʀᴇꜰᴇʀʀᴇʀ ᴏɴʟʏ)</b>\n"
        f"{ce('star')} {sc('ᴛᴏᴛᴀʟ ʀᴇꜰᴇʀʀᴀʟꜱ')}: <b>{u['referrals']}</b>\n"
        f"{ce('money')} {sc('ᴄʀᴇᴅɪᴛꜱ')}: <b>{u.get('credits', 0)}</b>{exp_txt}\n\n"
        f"{ce('warn')} {sc('ɴᴇᴡ ᴜꜱᴇʀ ᴍᴜꜱᴛ ᴊᴏɪɴ ᴀʟʟ ꜰᴏʀᴄᴇ ᴊᴏɪɴ ᴄʜᴀɴɴᴇʟꜱ!')}")
    await cq.message.edit_text(text, reply_markup=back_button(), parse_mode="HTML")
...
async def process_redeem(msg: types.Message, state: FSMContext):
    ...
    if not found.get("is_active", True):
        await msg.answer(f"{ce('cross')} {sc('ᴄᴏᴅᴇ ʜᴀꜱ ʙᴇᴇɴ ʀᴇᴠᴏᴋᴇᴅ.')}", parse_mode="HTML")
        return
    
    # Check expiry date
    if found.get("expiry_date"):
        try:
            if datetime.now() >= datetime.fromisoformat(found["expiry_date"]):
    ...
    if uid in found.get("used_by", []):
        await msg.answer(f"{ce('cross')} {sc('ʏᴏᴜ ᴀʟʀᴇᴀᴅʏ ᴜꜱᴇᴅ ᴛʜɪꜱ ᴄᴏᴅᴇ.')}", parse_mode="HTML")
        return
    
    # Check max users reached
    if found["used_count"] >= found.get("max_users", 1):
        await msg.answer(f"{ce('cross')} {sc('ᴄᴏᴅᴇ ʟɪᴍɪᴛ ʀᴇᴀᴄʜᴇᴅ.')}", parse_mode="HTML")
        return
    ...
    u = get_user(d, uid)
    credit_reward = found.get("credits", 0)
    u["credits"] = u.get("credits", 0) + credit_reward
    save(d)
    await state.clear()
    ...
    await msg.answer(styled_box("ᴄᴏᴅᴇ ʀᴇᴅᴇᴇᴍᴇᴅ ✅",
        ...
        f"{ce('money')} {sc('ᴄʀᴇᴅɪᴛꜱ')}: <b>+{credit_reward}</b>\n"
        f"{ce('diamond')} {sc('ʏᴏᴜʀ ʙᴀʟᴀɴᴄᴇ')}: <b>{u.get('credits', 0)}</b>"),
...
async def cb_premium_package(cq: types.CallbackQuery):
    ...
    text = styled_box("💎 ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ",
        f"{ce('gift')} {sc('ᴀᴍᴏᴜɴᴛ')}: <b>{credits} Credits</b>\n"
        f"{ce('money')} {sc('ᴘʀɪᴄᴇ')}: <b>₹{price_rs:.2f}</b>\n\n"
        f"{ce('info')} {sc('ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ ᴛᴏ ᴀᴅᴅ ʙᴀʟᴀɴᴄᴇ')}")
    
    await cq.message.edit_text(text, reply_markup=kb([[btn_primary("Contact Admin", url="https://t.me/astropulsesmmsupport", emoji_key="link")], [btn_danger("Back", callback="home", emoji_key="arrow")]]), parse_mode="HTML")
...
async def process_custom_credits(msg: types.Message, state: FSMContext):
    try:
        ...
        await msg.answer(text, reply_markup=kb([[btn_primary("Contact Admin", url="https://t.me/astropulsesmmsupport", emoji_key="link")], [btn_danger("Back", callback="home", emoji_key="arrow")]]), parse_mode="HTML")
    except:
...
async def cb_admin_stats(cq: types.CallbackQuery):
    ...
    rate = round(total_sent / total * 100, 1) if total else 0
    
    # Calculate user stats
    total_users = len(users)
    active_users = sum(1 for u in users.values() if u.get('daily_attacks', 0) > 0)
    premium_users = sum(1 for u in users.values() if u.get('credits', 0) > 1000)
    referred_users = sum(1 for u in users.values() if u.get('referred_by'))
    
    # Promo codes info
    active_codes = sum(1 for c in codes if c.get('is_active', False))
    total_code_uses = sum(c.get('used_count', 0) for c in codes)
    
    text = styled_box("ɢʟᴏʙᴀʟ ꜱʏꜱᴛᴇᴍ ꜱᴛᴀᴛᴀᴛᴀʜ",
        f"{ce('fire')} {sc('ɴᴏᴅᴇꜱ')}: <b>{len(d.get('firebases', []))}</b>\n"
        f"{ce('crown')} {sc('ᴀᴅᴍɪɴꜱ')}: <b>{len(d.get('admins', []))}</b>\n"
        f"{ce('cross')} {sc('ʙᴀɴɴᴇᴅ')}: <b>{len(d.get('banned', []))}</b>\n\n"
        f"{ce('eyes')} {sc('ᴛᴏᴛᴀʟ ᴜꜱᴇʀꜱ')}: <b>{total_users}</b>\n"
        f"{ce('green')} {sc('ᴀᴄᴛɪᴠᴇ ᴛᴏᴅᴀʏ')}: <b>{active_users}</b>\n"
        f"{ce('diamond')} {sc('ᴘʀᴇᴍɪᴜᴍ ᴜꜱᴇʀꜱ')}: <b>{premium_users}</b>\n"
        f"{ce('gift')} {sc('ʀᴇꜰᴇʀʀᴇᴅ ᴜꜱᴇʀꜱ')}: <b>{referred_users}</b>\n\n"
        f"{ce('green')} {sc('ᴛᴏᴛᴀʟ ꜱᴇɴᴛ')}: <b>{total_sent}</b>\n"
        f"{ce('red')} {sc('ꜰᴀɪʟᴇᴅ')}: <b>{total_failed}</b>\n"
        f"{ce('chart')} {sc('ꜱᴜᴄᴄᴇꜱꜱ ʀᴀᴛᴇ')}: <b>{rate}%</b>\n\n"
        f"{ce('diamond')} {sc('ᴀᴄᴛɪᴠᴇ ᴄᴏᴅᴇꜱ')}: <b>{active_codes}</b>\n"
        f"{ce('chart')} {sc('ᴄᴏᴅᴇ ʀᴇᴅᴇᴇᴍꜱ')}: <b>{total_code_uses}</b>")
...
async def cb_gen_code(cq: types.CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        styled_box("ɢᴇɴᴇʀᴀᴛᴇ ᴘʀᴏᴍᴏ ᴄᴏᴅᴇ",
        f"{ce('diamond')} {sc('ᴇɴᴛᴇʀ ᴄᴜꜱᴛᴏᴍ ᴄᴏᴅᴇ ɴᴀᴍᴇ')}\n\n"
        f"{ce('info')} {sc('ᴇxᴀᴍᴘʟᴇ')}: PREMIUM_PACK, VIP_BOOST"), parse_mode="HTML")
...
async def proc_code_name(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(f"{ce('money')} {sc('ʜᴏᴡ ᴍᴀɴʏ ᴄʀᴇᴅɪᴛꜱ ᴘᴇʀ ᴜꜱᴇ?')}\n\n{ce('info')} {sc('ᴇxᴀᴍᴘʟᴇ')}: 1000", parse_mode="HTML")
...
async def proc_code_sms(msg: types.Message, state: FSMContext):
    try:
        credits = int(msg.text.strip())
        if credits < 1:
            await msg.answer(f"{ce('cross')} {sc('ᴍɪɴɪᴍᴜᴍ 1 ᴄʀᴇᴅɪᴛ.')}", parse_mode="HTML")
            return
    ...
    await msg.answer(f"{ce('eyes')} {sc('ᴍᴀx ᴜꜱᴇʀꜱ ᴛʜᴀᴛ ᴄᴀɴ ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴅᴇ?')}\n\n{ce('info')} {sc('ᴇxᴀᴍᴘʟᴇ')}: 50", parse_mode="HTML")
...
async def proc_code_maxusers(msg: types.Message, state: FSMContext):
    try:
        ...
        if max_users < 1:
            await msg.answer(f"{ce('cross')} {sc('ᴍɪɴɪᴍᴜᴍ 1 ᴜꜱᴇʀ.')}", parse_mode="HTML")
            return
    ...
    await msg.answer(f"{ce('clock')} {sc('ᴇxᴘɪʀʏ ᴅᴀʏꜱ?')}\n\n{ce('info')} {sc('ᴇxᴀᴍᴘʟᴇ')}: 7 (ꜱᴇᴛ 0 ꜰᴏʀ ɴᴇᴠᴇʀ)")}, parse_mode="HTML")
...
async def proc_code_days(msg: types.Message, state: FSMContext):
    try:
        ...
        if days < 0:
            await msg.answer(f"{ce('cross')} {sc('ᴠᴀʟɪᴅ ɴᴜᴍʙᴇʀ.')}", parse_mode="HTML")
            return
    ...
    exp_display = f"ɴᴇᴠᴇʀ" if data['expiry_days'] == 0 else f"{data['expiry_days']} ᴅᴀʏꜱ"
    
    await msg.answer(
        styled_box("ᴘʀᴏᴍᴏ ᴄᴏᴅᴇ ɢᴇɴᴇʀᴀᴛᴇᴅ ✅",
        f"{ce('diamond')} {sc('ᴄᴏᴅᴇ')}: <code>{code}</code>\n"
        f"{ce('money')} {sc('ᴄʀᴇᴅɪᴛꜱ')}: <b>+{data['credits']}</b>\n"
        f"{ce('eyes')} {sc('ᴍᴀx ᴜꜱᴇʀꜱ')}: <b>{data['max_users']}</b>\n"
        f"{ce('clock')} {sc('ᴇxᴘɪʀʏ')}: <b>{exp_display}</b>"),
