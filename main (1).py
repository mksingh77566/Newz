...
def mk_btn(text: str, callback: str = None, url: str = None, emoji_key: str = None) -> InlineKeyboardButton:
    """Create InlineKeyboardButton with custom emoji (Bot API 9.0+)"""
    kwargs = {"text": tc(text)}
    if callback:
...
async def cmd_start(msg: types.Message, state: FSMContext):
                ...
                if not u.get("referred_by"):
                    ...
                    try:
                        await msg.bot.send_message(ref_uid,
                            ...
                            f"{ce('fire')} {sc('ʙᴏɴᴜꜱ')}: <b>+1 ᴀᴛᴛᴀᴄᴋ</b>"), parse_mode="HTML")
                    except: pass
        except: pass
    ...
    text = (
        f"╭━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╮\n"
        f"   {ce('fire')} <b>THE LOST SMS BOMBER</b> {ce('skull')}\n"
        f"╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯\n\n"
        f"{ce('bolt')} <b>Welcome, {user_name}</b>\n"
        f"{ce('star')} <i>Your gateway to unlimited power</i>\n\n"
        f"╭─── {ce('brain')} <b>ACCOUNT INFO</b> ───╮\n"
        f"┃ {ce('eyes')} <b>User:</b> <code>{user_name}</code>\n"
        f"┃ {ce('fire')} <b>Daily Attacks:</b> <code>{u['daily_attacks']}</code>\n"
        f"┃ {ce('gift')} <b>Referrals:</b> <code>{u['referrals']}</code>\n"
        f"┃ {ce('diamond')} <b>Bonus:</b> <code>+{u['bonus_attacks']}</code>\n"
        f"╰──────────────────────────────╯\n\n"
        f"╭─── {ce('rocket')} <b>FEATURES</b> ───╮\n"
        f"┃ {ce('check')} Multi-Device Bombing\n"
        f"┃ {ce('check')} Parallel Sending (10x Fast)\n"
        f"┃ {ce('check')} Number Protection\n"
        f"┃ {ce('check')} Auto Scheduler\n"
        f"╰──────────────────────────────╯\n\n"
        f"<i>{ce('info')} {sc('ᴜꜱᴇ /stop ᴛᴏ ᴄᴀɴᴄᴇʟ ᴀɴʏᴛɪᴍᴇ')}</i>\n"
        f"<i>{ce('crown')} ᴅᴇᴠ: {OWNER_USERNAME}</i>"
    )
...
async def cb_refer(cq: types.CallbackQuery):
    ...
    text = styled_box("ʀᴇꜰᴇʀʀᴀʟ ꜱʏꜱᴛᴇᴍ",
        ...
        f"{ce('gift')} {sc('ʙᴏɴᴜꜱ/ʀᴇꜰ')}: <b>+1 ᴀᴛᴛᴀᴄᴋ</b>\n"
...
async def main():
    ...
    try:
        await bot.send_message(OWNER_ID,
            ...
            f"{ce('fire')} {sc('ᴛʜᴇ ʟᴏꜱᴛ ʙᴏᴍʙᴇʀ')} {VERSION}\n"
